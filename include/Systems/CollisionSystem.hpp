...
public:
    ...
    void update(ee::ecs::World& _world, float _dt) override
    {
        for (auto& id : m_entities)
        {
            ...
            ee::math::Vector2<float> nextPos = transform.position + translation + col.offset;
            ...
            if (motion.velocity.x < 0 && (!isWalkableAt(left - 16, top + inset) || !isWalkableAt(left - 16, bottom - inset)))
                motion.velocity.x = 0;

            if (motion.velocity.y < 0 && (!isWalkableAt(left + inset, top) || !isWalkableAt(right - inset, top)))
                motion.velocity.y = 0;

            if (motion.velocity.x > 0 && (!isWalkableAt(right + 1.f, top + inset) || !isWalkableAt(right + 1.f, bottom - inset)))
                motion.velocity.x = 0;

            if (motion.velocity.y > 0 && (!isWalkableAt(left + inset, bottom + 1.f) || !isWalkableAt(right - inset, bottom + 1.f)))
                motion.velocity.y = 0;
        }
    }
};
